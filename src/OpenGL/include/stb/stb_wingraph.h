...
int stbwingraph_MessageBox(stbwingraph_hwnd win, unsigned int type, char *caption, char *text, ...)
{
   ...
   va_end(v);
   return MessageBox((HWND)win, buffer, caption, type);
}
...
void stbwingraph_ShowWindow(void *window)
{
   ...
   stbwingraph__window *z = (stbwingraph__window *) GetWindowLong((HWND)window, GWL_USERDATA);
           ...
           int width, int height, int fullscreen, int resizeable, int dest_alpha, int stencil)
{
   ...
   win = CreateWindow("zwingraph", text ? text : "sample", dwstyle,
                      ...
                      NULL, NULL, (HINSTANCE)stbwingraph_app, z);
...
{
   stbwingraph__window *z = (stbwingraph__window *) GetWindowLong((HWND)win, GWL_USERDATA);
   if (z)
      wglMakeCurrent(z->dc, z->rc);
}
...
int stbwingraph_MainLoop(stbwingraph_update func, float mintime)
{
   ...
   for(;;) {
      ...
      if (needs_drawing || is_animating) {
         ...
         if (stbwingraph_primary_window) {
            stbwingraph__window *z = (stbwingraph__window *) GetWindowLong((HWND)stbwingraph_primary_window, GWL_USERDATA);
            if (z && !z->active) {
               real = 0;
            }
            if (z)
...
void stbwingraph_SwapBuffers(void *win)
{
   ...
   z = (stbwingraph__window *) GetWindowLong((HWND)win, GWL_USERDATA);
   if (z && z->dc)
      SwapBuffers(z->dc);
}
#endif
